...
Container image for running keepalived, a high-availability solution providing load balancing and failover using VRRP.
...
## Integration with conntrackd

For connection tracking synchronization in HA failover scenarios, use the [conntrackd container](conntrackd.md) alongside keepalived.

A `notify-conntrackd.sh` script is included in the image at `/notify-conntrackd.sh` for integration with conntrackd.

See the [conntrackd documentation](conntrackd.md) for setup instructions.
